...
if (isset ($_POST['place_order'])) {
    ...
    if (isset ($_GET['get_id'])) {
        ...
        while ($fci = $verify_cart->fetch(PDO::FETCH_ASSOC)) {
            ...
            $insert_order = $con->prepare("INSERT INTO `orders` (`id`, `user_id`, `name`, `number`, `email`, `address`, `address_type`, `method`, `product_id`, `price`, `qty`) VALUES (?,?,?,?,?,?,?,?,?,?,?);");
            $insert_order->execute([uniq_id(), $user_id, $name, $phone, $email, $address, $adtype, $method, $fci['product_id'], $fci['price'], $fci['qty']]);
        }
...
if (isset($_POST['delete_product'])) {
    ...
    $num_cart_pros = $cart_pros->fetchColumn(); // Fetch the count of items

    if ($num_cart_pros > 1) { // Check if there are more than one items in the cart
        $proid = $_POST['product_id'];
        $delete_checkout = $con->prepare("DELETE FROM `cart` WHERE product_id = ?");
        $delete_checkout->execute([$proid]);
        $success_msg[]="successfully deleted the item on checkout as well as cart";
    }else{
        $warning_msg[]="cannot buy a null product must have more than item in cart to delete";
    }
}
                    ...
                    if (isset ($_GET['get_id'])) {
                        ...
                        while ($fetch_get = $select_get->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            <div class="flex flexy">
                                ...
                                <img style="width:200px; height:200px;" src="<?php echo  $fetch_get["image"]?>" alt="not found prodpic">
                                <div>
                                    <h3 class="name">
                                        <?= $fetch_get['name']; ?>
                                        </h3>
                        ...
                        if ($select_cart->rowCount() > 0) {
                            while ($fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                <div class="flex flexy">
                                    ...
                                    <div>
                                        <img style="width:200px; height:200px;" src="<?php echo $fetch_product['image']?>" alt="not found prod pic">
                                        <h3 class="name">
                                            <?= $fetch_product['name']; ?>
                                        </h3>
                                        <p class="price">
